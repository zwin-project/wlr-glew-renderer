...
glew_get_texture(struct wlr_texture *wlr_texture)
{
  assert(wlr_texture_is_glew(wlr_texture));
  return (struct wlr_glew_texture *)wlr_texture;
}

static bool
glew_texture_is_opaque(struct wlr_texture *wlr_texture)
{
...
static bool
check_stride(const struct wlr_glew_drm_pixel_format *fmt, uint32_t stride,
    uint32_t width)
{
  if (stride % (fmt->bpp / 8) != 0) {
    wlr_log(WLR_ERROR,
        "Invalid stride %d (incompatible with %d "
        "bytes-per-pixel)",
        stride, fmt->bpp / 8);
    return false;
  }
  if (stride < width * (fmt->bpp / 8)) {
    wlr_log(WLR_ERROR,
        "Invalid stride %d (too small for %d "
        "bytes-per-pixel and width %d)",
        stride, fmt->bpp / 8, width);
    return false;
  }
  return true;
}
...
static bool
glew_texture_write_pixels(struct wlr_texture *wlr_texture, uint32_t stride,
    uint32_t width, uint32_t height, uint32_t src_x, uint32_t src_y,
    uint32_t dst_x, uint32_t dst_y, const void *data)
{
  struct wlr_glew_texture *texture = glew_get_texture(wlr_texture);
  ...
  const struct wlr_glew_pixel_format *fmt =
      get_glew_format_from_drm(texture->drm_format);
  assert(fmt);

  const struct wlr_glew_drm_pixel_format *drm_fmt =
      get_glew_drm_format_from_drm(texture->drm_format);
  assert(drm_fmt);
  ...
  push_glew_debug(texture->renderer);
  ...
  glTexSubImage2D(GL_TEXTURE_2D, 0, dst_x, dst_y, width, height, fmt->gl_format,
      fmt->gl_type, data);
  ...
  pop_glew_debug(texture->renderer);
...
static bool
glew_texture_invalidate(struct wlr_glew_texture *texture)
{
...
void
glew_texture_destroy(struct wlr_glew_texture *texture)
{
...
static void
glew_texture_unref(struct wlr_texture *wlr_texture)
{
  struct wlr_glew_texture *texture = glew_get_texture(wlr_texture);
  if (texture->buffer != NULL) {
    ...
    glew_texture_destroy(texture);
  }
}

static const struct wlr_texture_impl texture_impl = {
    .is_opaque = glew_texture_is_opaque,
    .write_pixels = glew_texture_write_pixels,
    .destroy = glew_texture_unref,
};

static struct wlr_glew_texture *
glew_texture_create(
    struct wlr_glew_renderer *renderer, uint32_t width, uint32_t height)
{
  struct wlr_glew_texture *texture = calloc(1, sizeof(struct wlr_glew_texture));
  if (texture == NULL) {
...
glew_texture_from_pixels(struct wlr_renderer *wlr_renderer, uint32_t drm_format,
    uint32_t stride, uint32_t width, uint32_t height, const void *data)
{
  ...
  const struct wlr_glew_drm_pixel_format *drm_fmt =
      get_glew_drm_format_from_drm(drm_format);
  assert(drm_fmt);
  ...
  struct wlr_glew_texture *texture =
      glew_texture_create(renderer, width, height);
  if (texture == NULL) {
    return NULL;
  }
    ...
    struct wlr_renderer *wlr_renderer, struct wlr_dmabuf_attributes *attribs)
{
  ...
  struct wlr_glew_texture *texture =
      glew_texture_create(renderer, attribs->width, attribs->height);
  if (texture == NULL) {
    return NULL;
  }
  ...
  const struct wlr_glew_drm_pixel_format *drm_fmt =
      get_glew_drm_format_from_drm(attribs->format);
  if (drm_fmt != NULL) {
...
static void
texture_handle_buffer_destroy(struct wlr_addon *addon)
{
  struct wlr_glew_texture *texture =
...
static const struct wlr_addon_interface texture_addon_impl = {
    .name = "wlr_glew_texture",
    .destroy = texture_handle_buffer_destroy,
};

static struct wlr_texture *
glew_texture_from_dmabuf_buffer(struct wlr_glew_renderer *renderer,
    struct wlr_buffer *buffer, struct wlr_dmabuf_attributes *dmabuf)
{
  ...
  if (addon != NULL) {
    struct wlr_glew_texture *texture =
        wl_container_of(addon, texture, buffer_addon);
    if (!glew_texture_invalidate(texture)) {
  ...
  struct wlr_texture *wlr_texture =
      glew_texture_from_dmabuf(&renderer->wlr_renderer, dmabuf);
  if (wlr_texture == NULL) {
    return false;
  }
  ...
  wlr_addon_init(
      &texture->buffer_addon, &buffer->addons, renderer, &texture_addon_impl);

  return &texture->wlr_texture;
}

struct wlr_texture *
glew_texture_from_buffer(
    struct wlr_renderer *wlr_renderer, struct wlr_buffer *buffer)
{
  ...
  if (wlr_buffer_get_dmabuf(buffer, &dmabuf)) {
    return glew_texture_from_dmabuf_buffer(renderer, buffer, &dmabuf);
  } else if (wlr_buffer_begin_data_ptr_access(buffer,
                 WLR_BUFFER_DATA_PTR_ACCESS_READ, &data, &format, &stride)) {
    struct wlr_texture *tex = glew_texture_from_pixels(
